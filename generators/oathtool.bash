...
# set, we will use $(dirname $(realpath -s $0))/../etc/oathtool.keys. Set
# PROTOCOL (default tcp) to change it. The final output will be based on the
# number generated by the oathtool. In order to limit the port range (so that
# we won't take up ports used by other applications), we employ the following
# method.
#
# First, we mod the oathtool code by a power of ten to get a number that is
...
# defaults to 1). Given these default options, our port range is limited
# between 10000 and 19999.
#

set -e

keys_file="${KEYS_FILE:-$(dirname $(realpath -s $0))/../etc/oathtool.keys}"
